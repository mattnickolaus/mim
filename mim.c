...
int getCursorPostition(int *rows, int *cols) {
  char buf[32];
  unsigned int i = 0;
...
int getWindowSize(int *rows, int *cols) {
  ...
  if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
    ...
    return getCursorPostition(rows, cols);
  } else {
    *cols = ws.ws_col;
    *rows = ws.ws_row;
    return 0;
...
void editorUpdateSyntax(erow_t *row) {
...
int editorRowRxToCx(erow_t *row, int rx) {
  int cur_rx = 0;
  int cx;
  for (cx = 0; cx < row->size; cx++) {
    if (row->chars[cx] == '\t') {
      cur_rx += (MIM_TAB_STOP - 1) - (cur_rx % MIM_TAB_STOP);
    }
    cur_rx++;
...
void editorUpdateRow(erow_t *row) {
  ...
  free(row->render);
  row->render = malloc(row->size + tabs * (MIM_TAB_STOP - 1) + 1);

  int idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % MIM_TAB_STOP != 0) {
        row->render[idx++] = ' ';
      }
    } else {
...
void editorSave() {
  if (E.filename == NULL) {
    ...
    // User hits ESC out of the prompt
    if (E.filename == NULL) {
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
                                  "Mim editor -- version %s", MIM_VERSION);
...
void editorSetStatusMessage(const char *fmt, ...) {
  ...
  va_end(ap);
  E.statusmsg_time = time(NULL); // Gets the current time
}
